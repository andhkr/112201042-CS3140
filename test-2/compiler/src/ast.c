...
void op_Assign(node *new_node, symbltblentry *entry, node *left, node *right) {
  ...
  switch (left->type) {
  case TWODINTARRAY:
    // twodintarray i2array = left->entry->value.twodintarr;
    // node* row = left->ptr_children_list->ptr_sibling;
    // node* col = row->ptr_sibling;
    // // int r_ind = give_index(row);
    // // int c_ind = give_index(col);
    
    // int index1 = array_index(left);


    // if (index >= i2array.capacity) {
    //   fprintf(stderr, "error:index out of bounds\n");
    //   exit(EXIT_FAILURE);
    // }

    if (stm_stack.sp == 0) {
      // (((i2array.ptr)+r_ind)*+c_ind)* = right->exp_value.integer;
    }
    break;
  case TWODFLOATARRAY:
    break;
