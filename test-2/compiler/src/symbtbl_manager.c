...
void print_entry(symbltblentry *entry) {
  ...
  case FLOAT:
    printf("| %-*s | %-*s | %-*f |\n", clen_for_var, entry->name, clen_for_type,
           "float", clen_for_val, entry->value.sdecimal);
    break;
  case CHAR:
    printf("| %-*s | %-*s | %-*c |\n", clen_for_var, entry->name, clen_for_type,
  ...
  case FLOATARRAY:
    printf("| %-*s | %-*s | ", clen_for_var, entry->name, clen_for_type,
            "floatarray");
    for (int i = 0; i < entry->value.floatarr.capacity; ++i) {
      printf("%f , ", entry->value.floatarr.ptr[i]);
    }
    printf("|\n");
    break;
  case TWODINTARRAY:
    printf("| %-*s | %-*s | ", clen_for_var, entry->name, clen_for_type,
           "2dintarray");
    for (int i = 0; i < entry->value.twodintarr.row; ++i) {
      for(int j = 0;j<entry->value.twodintarr.col;++j){
          printf("%d , ", (entry->value.twodintarr.ptr[i])[j]);
      }
      printf("\n");
      for(int k = 0;k<(clen_for_type+clen_for_val+clen_for_var-3);++k){
        printf(" ");
      }
    }
    printf("|\n");
  ...
  case TWODFLOATARRAY:
    printf("| %-*s | %-*s | ", clen_for_var, entry->name, clen_for_type,
           "2dfloatarray");
    for (int i = 0; i < entry->value.twodfloatarr.row; ++i) {
      for(int j = 0;j<entry->value.twodfloatarr.col;++j){
          printf("%f , ", (entry->value.twodfloatarr.ptr[i])[j]);
      }
      printf("\n");
      for(int k = 0;k<(clen_for_type+clen_for_val+clen_for_var-3);++k){
        printf(" ");
