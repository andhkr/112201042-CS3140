...
%token <treeNode> WRITE DECL ENDDECL T_INT T_BOOL rt_FLOAT
...
%token <treeNode> FOR BREAK_Y DO WHILE
%token <entry> VAR
%token <i_var> NUM 
%token <f_var> T_FLOAT 
%token <b_var> T F
...
%%
	...
	ret_type:	T_INT   | T_BOOL | rt_FLOAT	{
												$$ = $1;
											}
		;
	...
	Gid	:	VAR		{ 
		...
		|   Gid '[' NUM ']' '[' NUM ']'{
									add_2Darray_to_symbtbl($1,$3,$6);
									node* arr = create_empty_node("2Darray declaration");
									char word1[13];
									snprintf(word1,sizeof(word1),"%d",$3);
									char word2[13];
									snprintf(word2,sizeof(word2),"%d",$6);
									node* row = create_empty_node(word1);
									node* col = create_empty_node(word2);
									arr->ptr_children_list = $1;
									$1->ptr_sibling = row;
									row->ptr_sibling = col;
									$$ = arr;
	...
	cond_stmt:	IF expr THEN stmt_list ENDIF{	
		...
		| DO '{' stmt_list '}' WHILE '(' expr ')' ';' {
			node* body = create_empty_node("do-while loop body");
			node* condn = create_empty_node("while condition");
			$1->ptr_children_list = body;
			body->ptr_children_list = $3;
			body->ptr_sibling = condn;
			condn->ptr_children_list = $5;
			$5->ptr_children_list = $7;
			pop_stmt(&stm_stack);
			$$ = $1;
	...
	expr	:	NUM 		{
		...
		|
			T_FLOAT			{
								char word[13];
								snprintf(word,sizeof(word),"%f",$1);
								node* num = create_empty_node(word);
								num->type = FLOAT;
								memset(&num->exp_value,0,sizeof(datavalue));
								num->exp_value.sdecimal = $1;
								$$ = num;
							}	
	...
	var_expr:	VAR	{			node* var_node = create_empty_node($1->name);
		...
		|	var_expr '[' expr ']'	{
										...
										index->entry = $3->entry;
										// make_node($1);
										int ind = give_index(index);
										char word[13];
										snprintf(word,sizeof(word),"%d",ind);
										index->ptr_children_list = create_empty_node(word);
		...
		|	var_expr '[' expr ']' '[' expr ']'{
										char* label = "2DArrayAccess";
										$1->label = strdup(label);
										node* r_index = create_empty_node("row_index");
										if($3->entry == NULL)
										r_index->exp_value.integer = $3->exp_value.integer;
										r_index->entry = $3->entry;
										int ind = give_index(r_index);
										char word[13];
										snprintf(word,sizeof(word),"%d",ind);
										r_index->ptr_children_list = create_empty_node(word);
										node* c_index = create_empty_node("col_index");
										if($6->entry == NULL)
										c_index->exp_value.integer = $6->exp_value.integer;
										c_index->entry = $6->entry;
										int ind2 = give_index(c_index);
										char word2[13];
										snprintf(word,sizeof(word2),"%d",ind2);
										c_index->ptr_children_list = create_empty_node(word2);
										node* left = create_empty_node($1->entry->name);
										$1->ptr_children_list = left;
										left->ptr_sibling = r_index;
										r_index->ptr_sibling = c_index;
										$1->type = $1->entry->type;
										$$ = $1;
									} 
		;
