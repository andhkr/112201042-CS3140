...
void add_array_to_symbtbl(
  ...
  case INT:
    ...
    break;
  case FLOAT:
    float *farr = (float*)calloc(n_ele, sizeof(float));
    var->entry->type = FLOATARRAY;
    floatarray farray;
    farray.capacity = n_ele;
    farray.ptr = farr;
    update_data(&var->entry->value.floatarr, &farray, sizeof(floatarray));
    break;
  default:
    break;
  }
...
void add_2Darray_to_symbtbl(
  ...
  switch (var->type) {
    case INT:
      int** arr = (int **)calloc(n_row, sizeof(int));
      var->entry->type = TWODINTARRAY;
      twodintarray array;
      array.row = n_row;
      array.col = n_col;
      array.ptr = arr;
      for(int i = 0;i<n_row;++i){
        arr[i] = (int*) calloc(n_col,sizeof(int));
      }
      update_data(&var->entry->value.twodintarr, &array, sizeof(twodintarray));
      break;
    case FLOAT:
      float** farr = (float **)calloc(n_row, sizeof(float));
      var->entry->type = TWODFLOATARRAY;
      twodfloatarray farray;
      farray.row = n_row;
      farray.col = n_col;
      farray.ptr = farr;
      for(int i = 0;i<n_row;++i){
        farr[i] = (float*) calloc(n_col,sizeof(float));
      }
      update_data(&var->entry->value.twodfloatarr, &farray, sizeof(twodfloatarray));
      break;
    default:
      break;
    }
